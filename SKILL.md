...
- 每个修改段先给一行【逻辑地图】，例如“`S 论点 → E 因果 → E 例证 → S2 置换 → C 回扣`”；再把修改版按句分行，并在句首标注其工种：`[S 论点]`、`[E 因果]`、`[E 例证]`、`[S2 置换推理]`、`[C 回扣]`。正负题额外可用 `[让步]`、`[权衡]`；双边题可用 `[正方]`、`[反方/驳论]`；分析解决题可用 `[原因]`、`[对应建议]`。标签只用于讲解版，全文整合版必须移除标签。
