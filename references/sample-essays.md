...
> 以下两篇为7分档基准：范文产出一律对齐此标注密度与论证链完整度。

### 精讲一｜同意与否类：强制社区服务

**题目**：Some people believe that unpaid community service should be a compulsory part of high school programmes (for example working for a charity, improving the neighbourhood or teaching sports). To what extent do you agree or disagree?

【题型判定】同意与否类（信号：To what extent do you agree or disagree）。

【立场路径】偏向式：同意为主，但开头先让一小步（承认学业负担的担忧），结尾再收回来。不走极端同意，显得有权衡能力。

**范文**

It is argued by some that unpaid community service ought to be a mandatory component of secondary education, such as working for charities or improving local neighbourhoods. While concerns about students' workload are not to be dismissed, I firmly believe that making such service compulsory is, on the whole, a wise educational policy.

【标注·开头】改写用了换搭配（should → ought to be a mandatory component）+ 概括（high school → secondary education）；While 让步句先堵对方嘴，再 I firmly believe 亮偏向式立场。
...
【标注·结尾】far outweigh 权衡句式 + 重述立场，不抄开头原句。

【本篇亮点】①shoulder 一词双关（承担责任/背负负担）；②complements rather than competes 头韵对比；③两主体段维度不重叠（品德 vs 技能）。

【讲解要点（讲师用）】①先让学生找三个 S（两段论点 + 让步靶子），体会"一段一论点"；②问学生：if 那句删掉行不行？——删掉论证就只剩正面自说自话，置换推理是 7 分的分水岭；③提醒：让步段只让一句，多让就翻车。
...
**题目**：Nowadays, a large amount of food is wasted and thrown away in many countries. What are the reasons for this problem? What measures can be taken to solve it?

【题型判定】分析解决类（信号：reasons + measures 双问）。

【立场路径】无需选边，但要保证"原因≥2 条、建议≥2 条"，且建议与原因对应（买多了→改促销；点多了→打包文化/教育）。

**范文**

It is a worrying phenomenon that an immense amount of food is discarded every year in both households and restaurants. There are some practical reasons why this problem has become severe, yet several feasible solutions can also be identified.
...
【标注·结尾】一句话压缩原因 + 一句话给出多方合力，the joint effort 复用。

【本篇亮点】①原因段与建议段严格对应；②buy one get one free、doggy bags 等具体化表达（具象化得分）；③It is high time 虚拟语气。
